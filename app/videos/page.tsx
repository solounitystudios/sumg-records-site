...
export default function VideosPage() {
  const resolvedVideos = getResolvedVideos(videos);
    ...
    <>
      ...
      <section className="bg-[#f7f7f5] py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          <SectionHeading eyebrow="Latest visuals" title={`${resolvedVideos.length} Entries`} />
          {resolvedVideos.length > 0 ? (
              ...
              {resolvedVideos.map((video) => (
                <article key={video.slug} className="border-t border-neutral-800/20 pt-5">
                  ...
                  <p className="mt-4 text-[10px] uppercase tracking-[0.23em] text-neutral-400">
                    {video.ownerType} · {video.ownerName} · {formatDate(video.date)}
                  </p>
            ...
            <div className="border border-neutral-800/15 p-8 bg-white">
              <p className="text-xl font-light tracking-tight text-neutral-900">
                Visual programming will be announced soon.
              </p>
            </div>
          )}
        </div>
